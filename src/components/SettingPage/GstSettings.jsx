...
export default function GstSettings() {
  ...
  if (loading) return <p className="text-sm text-gray-500">Loading GST settings...</p>;
  if (!formData) return <p className="text-sm text-red-500">{error || "Unable to load GST settings."}</p>;
      ...
      {formData.isGstRegistered && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <FormField label="GSTIN" name="gstin" value={formData.gstin} onChange={handleChange} placeholder="15-digit GSTIN" error={fieldErrors.gstin} />
            <FormField label="Legal Business Name" name="legalBusinessName" value={formData.legalBusinessName} onChange={handleChange} error={fieldErrors.legalBusinessName} />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <FormField label="GST Rate (%)" name="gstRate" type="number" value={formData.gstRate} onChange={handleChange} error={fieldErrors.gstRate} />
            <FormField label="SAC Code" name="sacCode" value={formData.sacCode} onChange={handleChange} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <FormField label="State Code" name="stateCode" value={formData.stateCode} onChange={handleChange} placeholder="e.g. 32 for Kerala" error={fieldErrors.stateCode} />
          </div>
        </>
      )}
