...
import { getBranchApi, updateBranchApi } from "../../../api/api";
...
export default function EditBranchModal({ branch, onClose, onBranchUpdated }) {
  ...
  useEffect(() => {
    async function loadBranch() {
      try {
        const res = await getBranchApi(branch.id);
        const b = res.data;
        const s = b.lab_setting || {};
        setFormData({
          name: b.name || "",
          code: b.code || "",
  ...
  if (!formData) {
    return (
      <ModalShell title="Edit Branch" onClose={onClose}>
        <div className="p-6">
          <p className="text-sm text-red-600">{error || "Unable to load branch details."}</p>
        </div>
      </ModalShell>
    );
  }
    ...
    <ModalShell title="Edit Branch" onClose={onClose}>
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="branch-active" className="text-sm text-gray-700">
            Active
          </label>
        </div>
